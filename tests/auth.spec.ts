...
test('Veracity authentication flow works as expected for external users', async ({ page }) => {
  ...
  await expect(page.getByText(email?.toLowerCase() as string)).toBeVisible(increasedTimeout)
})
